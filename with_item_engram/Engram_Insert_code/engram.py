...
@dataclass
class EngramConfig:
    tokenizer_name_or_path: str = "Qwen3-1.7B"
    # field 为数据类的字段配置高级属性
...
class MultiHeadEmbedding(nn.Module):
    ...
    def _init_embedding(self):
        """初始化嵌入权重（训练模式）"""
        #nn.init.normal_(self.embedding.weight, mean=0.0, std=1.0 / np.sqrt(self.embedding_dim))
...
class Engram(nn.Module):
    def __init__(self, 
        ...
        # 推理增量缓存（在 generate + use_cache=True 时使用）
    ...
    def reorder_inference_cache(self, beam_idx: torch.LongTensor):
        """
        beam search 选枝后按 beam_idx 重排 Engram 内部缓存，防止不同 beam 历史状态串线。
        """
        if beam_idx is None:
            return
        ...
        if self._token_hist_cache is not None:
            ...
            elif src.size(0) > 0 and idx.size(0) % src.size(0) == 0:
                repeat = idx.size(0) // src.size(0)
                src_expanded = src.repeat_interleave(repeat, dim=0)
                self._token_hist_cache = src_expanded.index_select(0, idx).detach()
            else:
                raise RuntimeError(
                    f"Engram token cache batch mismatch: cache={src.size(0)}, beam_idx={idx.size(0)}"
                )
        ...
        if self._value_hist_cache is not None:
            ...
            if src.size(0) == idx.size(0):
                ...
                raise RuntimeError(
                    f"Engram value cache batch mismatch: cache={src.size(0)}, beam_idx={idx.size(0)}"
                )
    ...
    def _forward_full(self, hidden_states: torch.Tensor, input_ids: torch.Tensor,item_attention_mask: torch.Tensor | None = None,):
        ...
        #print("嵌入层:",embeddings[0, -1, :10])
        #print("gate:",gate[0, -1, :])
        #print("value:",value[0, -1, :10])
        #print("output:",output[0, -1, :10])

        return output, gate, value

    def _forward_incremental(self, hidden_states: torch.Tensor, input_ids: torch.Tensor, item_attention_mask: torch.Tensor | None = None,):
        ...
        conv_out, new_value_hist = self.short_conv.forward_with_history(
                x_cur=value,
                x_hist=self._value_hist_cache,
            )
    ...
    def forward(
        ...
        if self.training or self.multi_head_embedding._emb_weight is None:
            ...
            output, gate, _ = self._forward_full(hidden_states, input_ids,item_attention_mask)
            return output, gate

        # 推理时，如果不用 cache，直接正常全部计算
        if not use_cache:
            output, gate, _ = self._forward_full(hidden_states, input_ids,item_attention_mask)
            #print("该批次的第一个数据的最后几个id为:",input_ids[0,-5:])
            return output, gate

        # generate 第一轮：整段 prompt 跑一遍，并初始化内部缓存
        if is_first_iteration:
            self.reset_inference_cache()
            output, gate, value = self._forward_full(hidden_states, input_ids,item_attention_mask)
        ...
        return self._forward_incremental(hidden_states, input_ids, item_attention_mask)
    ...
    def _update_token_hist_cache(self, ids_cur: torch.Tensor):
        ...
        #print("该批次的第一个数据的cache为:",self._token_hist_cache[0,:])
...
class TransformerBlock(nn.Module):
    def __init__(self,layer_id):
        super().__init__()
        self.attn = lambda x:x  # 占位注意力层，实际应替换为真实MultiHeadAttention
        self.moe  = lambda x:x  # 占位MoE层，实际应替换为真实MoE实现
        self.engram = None
        # 仅当当前图层ID在engram的配置列表中，才实例化Engram模块
...
if __name__ == '__main__':
    LLM = [
        ...
        # Transformer Blocks
        # *：Python 解包运算符，将列表中的多个 TransformerBlock 实例解包为单独的元素，
        # 按顺序添加到 LLM 总列表中，实现「多层级联」的效果
    ...
    # 对文本做分词处理，return_tensors='pt'表示输出PyTorch 张量，提取分词后的整数 Token ID序列
    ...
    for idx, layer in enumerate(LLM):
        if idx == 0:
            # 单分支版本：词嵌入层输出直接为 [B, L, D]，无多分支扩展
            hidden_states = LLM[0](input_ids)
            # 移除多分支扩展逻辑
        elif idx == len(LLM)-1:
            # 单分支版本：直接使用 hidden_states 输出，无需选取分支
