...
CHECKPOINT_PATH="${CHECKPOINT_PATH:-./output_sft_layer=6,13,20_lr=1e-4/final_checkpoint_sft}"
MASTER_PORT="${MASTER_PORT:-49225}"
LOG_FILE="${LOG_FILE:-eval_sft_engram.log}"

if [[ ! -d "$CHECKPOINT_PATH" ]]; then
    echo "[Error] checkpoint not found: $CHECKPOINT_PATH"
    echo "        Set CHECKPOINT_PATH=./output_sft_layer=6,13,20_lr=1e-4/checkpoint-XXXX or run sft.sh first."
    exit 1
fi
