...
from transformers import AutoTokenizer, BitsAndBytesConfig, GenerationConfig, LogitsProcessorList


PARENT_DIR = os.path.dirname(os.path.dirname(os.path.abspath(__file__)))
DATA_ROOT = os.path.join(os.path.dirname(PARENT_DIR), "data")
DEFAULT_TEST_FILE = os.path.join(DATA_ROOT, "Industrial_and_Scientific_dataset", "valid.jsonl")
DEFAULT_INFO_FILE = os.path.join(DATA_ROOT, "Amazon", "info", "Industrial_and_Scientific_5_1996-10-2018-11.txt")
...
from modeling_qwen3 import Qwen3ForCausalLM  # noqa: E402
...
def _capture_full_layers(model):
    ...
    return (
        base_model,
        list(base_model.model.layers),
        list(getattr(base_model.model, "engram_layer_ids", [])),
    )


def _truncate_model_to_layer(base_model, full_layers, full_engram_layer_ids, target_layer: int):
    ...
    base_model.model.engram_layer_ids = [
        layer_id for layer_id in full_engram_layer_ids if layer_id <= target_layer
    ]
    return base_model
...
def _load_engram_params(model, checkpoint_dir: str):
    engram_params_dir = os.path.join(checkpoint_dir, "engram_params")
    if not os.path.exists(engram_params_dir):
        print(f"⚠️ 未找到Engram参数目录: {engram_params_dir}", flush=True)
        return
    ...
    for _, module in unwrapped_model.named_modules():
        if isinstance(module, Engram):
            ...
            if os.path.exists(param_path):
                ...
                print(f"⚠️ Engram层 {layer_id} 参数文件缺失: {param_path}", flush=True)
    print(f"\n📌 总计加载 {engram_loaded} 个Engram层参数", flush=True)


def _convert_engram_params_to_float32(model):
...
def inference(
    ...
    if resume_from_checkpoint:
        ...
        if engram_float32:
            model = _convert_engram_params_to_float32(model)
        _load_engram_params(model, resume_from_checkpoint)

    base_causal_lm, full_layers, full_engram_layer_ids = _capture_full_layers(model)
    ...
    for zero_based_layer in target_layer_ids:
        ...
        truncated_model = _truncate_model_to_layer(
            ...
            full_engram_layer_ids=full_engram_layer_ids,
        ...
        for step in range(steps):
            ...
            if hasattr(base_causal_lm, "reset_engram_inference_cache"):
                base_causal_lm.reset_engram_inference_cache()
            ...
            running_metrics = _compute_metrics(all_preds, targets[: len(all_preds)], topk)
            running_metric_text = " | ".join(
                [
                    f"Hit@{k}: {running_metrics[f'Hit@{k}']:.6f} | NDCG@{k}: {running_metrics[f'NDCG@{k}']:.6f}"
                    for k in topk
                ]
            )
            print(
                ...
                f"samples_done: {len(all_preds)}/{total} | {running_metric_text}",
                flush=True,
            )
...
def _build_arg_parser():
    ...
    parser.add_argument("--engram_layer_ids", type=str, default="")
    parser.add_argument("--engram_float32", type=_str_to_bool, default=False)
    return parser
