...
class Engram(nn.Module):
    ...
    def _forward_full(self, hidden_states: torch.Tensor, input_ids: torch.Tensor):
        ...
        return output, gate, value, embeddings
    ...
    def forward(
        ...
        if self.training or self.multi_head_embedding._emb_weight is None:
            self.multi_head_embedding._emb_weight = None
            output, gate, _, embeddings = self._forward_full(hidden_states, input_ids)
            return output, gate, embeddings
        ...
        if is_first_iteration:
            ...
            output, gate, value, embeddings = self._forward_full(hidden_states, input_ids)
